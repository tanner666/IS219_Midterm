...
"""@pytest.mark.parametrize("a, operation, b, expected", [
    ...
    (Decimal('10'), divide, Decimal('2'), Decimal('5')),  # Test division
    (Decimal('4'), log, Decimal('2'), Decimal('2')),  # Test log two parameters
    (Decimal('10.5'), add, Decimal('0.5'), Decimal('11.0')),  # Test addition with decimals
    (Decimal('10.5'), subtract, Decimal('0.5'), Decimal('10.0')),  # Test subtraction with decimals
...
def test_calculation_operations(a, operation, b, expected):
    ...
    """
    calc = Calculation(a, operation, b)  # Create a Calculation instance with the provided operands and operation.
    assert calc.perform_two_operands() == expected, f"Failed {operation.__name__} operation with {a} and {b}"  # Perform the operation and assert that the result matches the expected value.
...
def test_calculation_repr():
    ...
    """
    calc = Calculation(Decimal('10'), add, Decimal('5'))  # Create a Calculation instance for testing.
    expected_repr = "Calculation(10, add, 5)"  # Define the expected string representation.
...
def test_calculation_repr_one():
    """Test for string representation of one operand operations"""
...
def test_invalid_logarithm():
    """
    Test logarithm with zero / neg number to ensure it raises a ValueError.
